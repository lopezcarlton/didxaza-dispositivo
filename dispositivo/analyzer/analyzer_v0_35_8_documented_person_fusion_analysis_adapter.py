#!/usr/bin/env python3
"""Analyzer v0.35.8: promote source-backed 1SG fusion candidates only when
...
def _orthographically_licensed_grave_final_i(
    headword_raw: str,
...
class DocumentedPersonFusionAnalysisAnalyzer:
    ...
    def analyze(
        ...
        *,
        ...
        exact_unresolved = set(
            result.get("still_exactly_unresolved_token_indexes", [])
        )
        exact_effective = set(
            result.get(
                "effective_evidence_after_voces_documentary_exact_token_indexes",
        ...
        for candidate in result.get(
            "supplemental_documented_person_fusion_candidates", []
        ):
        ...
        result.update(
            {
                ...
                "documented_person_fusion_analyzed_token_indexes": sorted(
                    analyzed_indexes
                ),
                "still_exactly_unresolved_token_indexes": sorted(exact_unresolved),
                "unresolved_token_indexes_after_documented_morphology": (
                    unresolved_after
                ),
                "effective_analysis_token_indexes_after_documented_morphology": (
                    effective_analysis
                ),
                "effective_analysis_token_count_after_documented_morphology": len(
                    effective_analysis
                ),
        ...
        result.setdefault("limitations", []).extend(
            [
                ...
                "ONLY_GP_1SG_GRAVE_FINAL_I_TO_E_GLOTTAL_IS_PROMOTED_IN_DOCUMENTED_MORPHOLOGY_LAYER",
